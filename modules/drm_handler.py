...
if m.document:
    ...
    # --- SAFE parsing of user selection (supports single index OR range like 1-10 OR padded like 001-079) ---
    raw_text = raw_text.strip()
    download_range = None

    # Case A: range input e.g. "1-10" or "001-079"
    if "-" in raw_text:
        try:
            start_s, end_s = raw_text.split("-", 1)
            start = int(start_s)
            end = int(end_s)
            if start < 1 or end < start or end > len(links):
                await editable.edit("⚠️ Invalid range. Use like: 1-10")
                return
            download_range = (start, end)
        except Exception:
            await editable.edit("⚠️ Invalid range format. Use numbers like: 1-10")
            return
    ...
    # Case B: single index
        ...
        try:
            single = int(raw_text)
            if single < 1 or single > len(links):
                await editable.edit(f"⚠️ Enter number between 1 and {len(links)}")
                return
            download_range = (single, len(links))
        except Exception:
            await editable.edit("⚠️ Invalid number input")
            return

    # Case C: default / invalid
    else:
        await editable.edit("⚠️ Invalid input. Use 5 or 1-10")
        return
...
# Determine start/end indices from download_range if set (document flow) else default single
if m.document:
    ...
    # text input flow: we always start from 1
    start_index = 1
    end_index = len(links)
    ...
    for idx in range(start_index - 1, end_index):
        ...
        try:
            ...
            if "drive" in url:
                ...
                try:
                    if 'api_url' in globals() or 'api_url' in globals().get('__dict__', {}):
                        await helper.pdf_download(f"{api_url}utkash-ws?url={url}&authorization={api_token}",f"{name}.html")
                    else:
                        await helper.pdf_download(f"utkash-ws?url={url}", f"{name}.html")
                    time.sleep(1)
